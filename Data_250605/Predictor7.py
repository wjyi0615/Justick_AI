...
class Predictor7:
    ...
    def save(self, rate, item, dir_path):
        ...
        torch.save(self.model.state_dict(), f"{dir_path}/{prefix}_model7.pth")
        joblib.dump(self.scaler_x, f"{dir_path}/{prefix}_scaler_x7.pkl")
        joblib.dump(self.scaler_y, f"{dir_path}/{prefix}_scaler_y7.pkl")

    def load(self, rate, item, dir_path):
        prefix = self._get_prefix(rate, item)
        self.model.load_state_dict(torch.load(f"{dir_path}/{prefix}_model7.pth"))
        self.model.eval()
        self.scaler_x = joblib.load(f"{dir_path}/{prefix}_scaler_x7.pkl")
        self.scaler_y = joblib.load(f"{dir_path}/{prefix}_scaler_y7.pkl")
        self.ewc_list = []
    ...
    def predict_next_week(self):
        return [int(price) for (_, price, _) in self.results[-7:]]

    def predict_next(self):
        return int(self.results[-7][1])
