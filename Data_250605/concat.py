...
# Step 1: 28일 예측
for item in items:
    df = pd.read_csv(f"Data_250605/store/{item}_separated.csv")
    for grade in grades:
        model = Predictor28()
        ...
        pd.DataFrame(pred_28).to_csv(f"Data_250605/store/{item}_{grade}_test.csv", index=False)
...
for item in items:
    ...
    for grade in grades:
        ...
        path = f"Data_250605/store/{item}_{grade}_test.csv"
